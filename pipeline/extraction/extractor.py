...
class GraphRAGExtractor(TransformComponent):
    ...
    def __init__(self, llm: Optional[LLM] = None, en_extract_prompt: Optional[Union[str, PromptTemplate]] = None,
                 rel_extract_prompt: Optional[Union[str, PromptTemplate]] = None, parse_en_fn: Callable = default_parse_triplets_fn,
                 parse_rel_fn: Callable = default_parse_triplets_fn, max_paths_per_chunk: int = 50, num_workers: int = 4) -> None:
        ...
        super().__init__(llm=llm or Settings.llm, en_extract_prompt=en_extract_prompt or KG_ENTITY_EXTRACT_TMPL, 
                         rel_extract_prompt=rel_extract_prompt or KG_RELATION_EXTRACT_TMPL, parse_en_fn=parse_en_fn, 
                         parse_rel_fn=parse_rel_fn, num_workers=num_workers, max_paths_per_chunk=max_paths_per_chunk)
    ...
    async def _aextract(self, node: BaseNode) -> BaseNode:
        ...
        for subj, obj, rel, description in relations:
            ...
            rel_node = Relation(label=canon(rel), source_id=subj_node.id, target_id=obj_node.id, properties=metadata)
